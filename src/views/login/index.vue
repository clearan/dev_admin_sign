                        ...
                        _this.$http.post('signin',qs.stringify(data)).then((resp)=>{
                            ...
                            if(resp.ret_code >= 0) {
                               // console.log(resp);return
                                ...
                                LocalStorage.set("sess", resp.ret_data.sess, date + 24*60*60*1000);
                                LocalStorage.set("self_id", resp.ret_data.self_id, date + 24*60*60*1000);
                                LocalStorage.set("user_name", _this.loginForm.username, date + 24*60*60*1000);
                                LocalStorage.set("user_info", resp.ret_data.userinfo, date + 24*60*60*1000);

                                let prex = ''
                                if (this.redirect === undefined) {
                                    prex = process.env.VUE_APP_PUSH_URL+'/main'
                                } else {

                                    if (this.redirect === '/apply/index' || this.redirect === '/apply/success') {
                                        prex = process.env.VUE_APP_PUSH_URL+'/main'
                                    } else {
                                        prex = process.env.VUE_APP_PUSH_URL+'/main#'+this.redirect
                                    }
                                }
